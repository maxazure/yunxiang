...
<script>
import { getSalesOrder, putSalesOrder } from '@/api/sales_order';
...
export default {
  ...
  methods: {
    async get() {
      const response = await getSalesOrder(this.$route.query.id);
      this.sales_orderForm = response.data;
    },
//    getApiList
    ...
    async api() {
      ...
      const res = await putSalesOrder(this.sales_orderForm.id,this.sales_orderForm);
    },
